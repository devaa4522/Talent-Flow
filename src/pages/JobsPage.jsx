...
//   React.useEffect(()=>{ API.get('/jobs?page=1&pageSize=50').then(r=>setJobs(r.items)) },[])
//   return (
//     <div className="card">
...
//     transition,
//     isDragging,
//   } = useSortable({ id: String(job.id) }); // IDs as strings
...
//   React.useEffect(() => {
//     API.get("/jobs?page=1&pageSize=50").then((r) => setJobs(r.items || []));
//   }, []);
...
//     const oldIndex = jobs.findIndex((j) => String(j.id) === String(active.id));
//     const newIndex = jobs.findIndex((j) => String(j.id) === String(over.id));
//     if (oldIndex < 0 || newIndex < 0) return;

//     // 1) Rearrange locally so the UI updates immediately
//     const before = jobs;
//     const reordered = arrayMove(jobs, oldIndex, newIndex);
//     setJobs(reordered);

//     // 2) OPTIONAL: persist to backend and refresh; rollback on failure
//     API.reorderJobs(reordered.map((j) => j.id))
//       .then(() => API.get("/jobs?page=1&pageSize=50").then((r) => setJobs(r.items || reordered)))
...
//           <SortableContext
//             items={jobs.map((j) => String(j.id))} // IDs as strings
...
//       {/* Minimal CSS for the drag handle; move to your stylesheet if you prefer */}
//       <style>{`
//         .drag-handle {
//           cursor: grab;
//           width: 100%;
...
//   // Load everything once; adjust the pageSize limit if your API caps it.
//   React.useEffect(() => {
...
export default function JobsPage() {
  ...
  async function handleCreate(values) {
    // Basic payload shape; adjust to your API
    const body = {
    ...
    const created = await API.post("/jobs", body); // should return the created job (with id)
    if (created && created.id) {
      // Prepend to list (or refetch)
      setJobs((prev) => [created, ...prev]);
      setShowCreate(false);
      // optionally jump to first page to see it:
      setPage(1);
